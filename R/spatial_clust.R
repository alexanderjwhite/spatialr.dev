...
#'
#' @examples 
spatial_clust <- function(input, u_init, v_init, w = NULL, lambda = 1, k = 20, epsilon = 1e-3, maxiter = 1e3){
  ...
  fct_c_optimize(x = x, u = u_init, v = v_init, w = w, index = index, lambda = lambda, epsilon = epsilon, maxiter = maxiter, display_progress = TRUE)
