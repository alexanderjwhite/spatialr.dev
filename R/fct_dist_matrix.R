...
fct_dist_matrix <- function(coords, distance = "euclidean", method = "dist", k = NULL, alpha = 1, verbose = TRUE){
  ...
  if(method == "dist"){
    ...
    adj_matrix <- as.matrix(Matrix::sparseMatrix(i = rep(1:nrow(coords), each = k), j = neighbours, x = 1, dims = c(nrow(coords), nrow(coords))))
    if(method == "knn_1"){
      w <- adj_matrix
    } else if(method == "knn_2"){
      w <- adj_matrix*as.matrix(exp(-alpha*dist_matrix))
    }
  }
